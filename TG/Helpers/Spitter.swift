...
//  Spitter.swift
//
//  Created by Andrii Narinian on 2/22/17.
...
class Spitter {
    ...
    static func showOkAlert(_ message: String?, title: String? = nil, viewController: UIViewController) {
        var permission = true
        blackList.forEach({ if message?.range(of: $0) != nil { permission = false }})
        if permission {
            showAlert(title, message: message, buttonTitles: ["Ok"], actions: [nil], owner: viewController)
        }
    }
    ...
    private static func displayWord (word: String, withColor color: UIColor, vc: UIViewController, completion: @escaping () -> Void) {
        ...
        vc.view.bringSubview(toFront: label)
        UIView.animate(withDuration: 0.6, delay: 0, options: .curveEaseOut, animations: {
            label.alpha = 0
    ...
    static func handleErrorStringClosure() -> (String) -> Void {
        return {errStr in
            var permission = true
    ...
    static func showOkAlertOnPVC(_ message: String) {
        if var topController = UIApplication.shared.keyWindow?.rootViewController {
            while let presentedViewController = topController.presentedViewController {
                topController = presentedViewController
            }
    ...
    static func showInputAlert(title: String? = nil,
                               ...
                               textfieldConfigurationHandler: @escaping ((UITextField) -> Void)) {
    ...
    static func activityCompletionHandler(activityType: UIActivityType?, completed: Bool, items: [Any]?, activityError: Error?) {
        if completed && activityError == nil {
            switch activityType! {
            case UIActivityType.mail:print("activity finished for mail")
            case UIActivityType.message:print("activity finished for message")
            case UIActivityType.postToFacebook:print("activity finished for FB")
            case UIActivityType.postToTwitter:print("activity finished for TWITTER")
            case UIActivityType.postToFlickr:print("activity finished for Flickr")
            case UIActivityType.postToVimeo:print("activity finished for Vimeo")
            case UIActivityType.postToWeibo:print("activity finished for Weibo")
